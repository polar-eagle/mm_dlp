...
class controlThread(QThread):
    ...
    def __init__(self):
        ...
        self.mmdlp = hardware.MM_DLP(-4.8, 29.52, 5.0, 3134676)
        self.condition = QWaitCondition()
        self.mutex = QMutex()
        self.command = None
        self.sliceName = None
...
class sliceThread(QThread):
    ...
    def work(self):
        ...
        f.write('plate %.2f %.2f %.2f %.2f\n' % (20.0, self.config['z_acc'], self.config['z_dec'], self.config['z_speed']))
    ...
    def printLayer(self, tank, layer, f):
        ...
        if self.now_tank != tank:
            f.write(
                'plate %.2f %.2f %.2f %.2f\n' % (5.0 + height, self.config['z_acc'], self.config['z_dec'], self.config['z_speed']))
            f.write('plate %.2f %.2f %.2f %.2f\n' % (
                95.0 + height, self.config['z_acc2'], self.config['z_dec2'], self.config['z_speed2']))
            f.write('glass -50 %.2f %.2f %.2f\n' % (self.config['z_acc2'], self.config['z_dec2'], self.config['z_speed2']))
            f.write('wait %d\n' % (30 if layer < 50 else 5))
            # --------清洗---------
        ...
        f.write('proj %s %.2f %d\n' % (
            dst_img_path,
            self.config['bottom_exposure_time_%d' % self.now_tank] if layer <= self.config['bottom_layers'] else self.config[
                'standard_exposure_time_%d' % self.now_tank],
            self.config['bottom_exposure_current_%d' % self.now_tank] if layer <= self.config['bottom_layers'] else self.config[
